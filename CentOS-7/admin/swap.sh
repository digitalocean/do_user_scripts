...
# Tell user the results of the script.
  if [[ "${?}" != "0" ]]; then
    printf "[ERROR] Failed to change file \"${swap_file_name}\" into a persistent swap file.\n";
    show_swaps;
    exit 1;
  else
    printf "[INFO] Successfully changed regular file \"${swap_file_name}\" into a persistent swap file.\n";
    show_swaps;
    exit 0;
  fi
